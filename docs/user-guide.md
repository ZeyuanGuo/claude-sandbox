...
所有日常操作都从同一个命令入口执行，具体操作由参数决定：

```bash
sbc                         # 进入 shell
sbc doctor                  # 只读快速诊断
sbc doctor --json           # 以 JSON 输出诊断结果
sbc recover                 # 自动选择启动或恢复审计
sbc audit restart           # 不重启容器，只恢复审计和目标网络
sbc status                  # 查看本实例容器状态
sbc audit status            # 查看当前审计进程和 PCAP 状态
```

帮助信息运行 `sbc --help`；某个子命令的详细参数运行 `sbc <子命令> --help`。新主机没有这个入口时，按根目录 [readme.md](../readme.md) 的“最短命令”一节配置 `~/.bashrc`。
...
默认配置生成器会创建宿主 `~/claude_code_config`，把 Claude 的提示词、设置、规则、skills 和 agents 分项读写挂载到容器 `~/.claude`，但不共享凭据、会话、历史和插件状态。宿主默认 `~/.claude` 不作为挂载源，直接运行 Claude Code 时不会自动读取共享目录。生成器也会自动创建并读写挂载宿主的 `~/.codex`，并在这些路径实际存在时同路径接入 `~/.agents/skills`、`~/.config/git` 和 `.condarc`；`.ssh` 与 `~/.local/share/gamma-ssh` 只读以保护关键私钥和 SSH 配置。
...
重启或断网后的第一条命令始终是 `sbc doctor`。如果它报告基础审计失活而四个容器仍在运行，执行 `sbc audit restart`；如果不确定容器是否被 Docker 保留，执行 `sbc recover`。这些恢复命令不会扩大到其他用户的容器或进程。
...
日常模式不会为普通公网 Web 请求创建逐条审核记录。此时访问失败应先检查网关、DNS、审计状态和明确阻断规则；未经 `host.yaml` 精确登记的直接 IP、SSH、UDP、外部 DNS 和非 Web 协议失败属于预期边界。登记的 SSH 失败时，检查容器是否已按最新配置重新初始化、宿主 Tailscale 接口和路由是否正常。容器内不设置 `HTTP_PROXY`、`HTTPS_PROXY` 或自定义 DNS。
