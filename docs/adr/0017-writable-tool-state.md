...
SSH 目录仍只读，因为其中包含私钥和 `authorized_keys` 等关键凭据。Claude 的 `.claude` 会话数据库仍保留在沙箱持久 Home；生成的 `CLAUDE.md`、`.bashrc`、`.profile`、`.gitconfig` 和 `.tmux.conf` 仍以受控只读副本挂入，防止容器绕过网络中和规则直接改写入口配置。
